...
$fichier=AuthUsr_DAO::isLoggedOn("vue/vue_listeReglements");
...
if($lesEcheances !== array()){
    ...
    for($i=0;$i<sizeof($lesEcheances);$i++){
        if($lesEcheances[$i]->getAttribut('etat') === 'EV'){
            $tabRows.='<tr>';

            $tabRows .= '<td><input form="exportValid" class="checkExport" type="checkbox" name="'.$lesEcheances[$i]->getAttribut('id').'" value=""></td>';
